...
__global__ void __launch_bounds__(kCudaBlockThreads, 1) resetScalarsKernel(
    ...
    const float gasSource = p.sceneSourceMode == 2 ? bed : 0.0f;
    const float materialFuel = p.sceneSourceMode == 2 ? gasSource * 0.16f : material * (p.sceneInitialFuelBase + chunkNoise * p.sceneInitialFuelNoise);
    heat[idx] = 0.0f;
