...
Require-Text $cuda "volumeVisibility = volumeShadowRay" "product shading no longer gates irradiance through volumetric visibility"
...
Require-Text $header "meanRoomIrradiance" "public frame metrics no longer expose mean product irradiance"
Require-Text $main "finalMeanVolumetricShadow" "validation JSON no longer reports mean volumetric shadow"
Require-Text $main "finalMeanRoomIrradiance" "validation JSON no longer reports mean product irradiance"
...
if (-not $SkipDiagnostics) {
    ...
    Require-Text $diag "volumetricShadowing=scene shadow volume stores soot optical transmittance and product rays sample source-to-surface visibility" "diagnostics do not report volumetric shadowing"
    Require-Text $diag "productLightingLayer=methanol floor/wall response receives flame-fed irradiance gated by volumetric shadow and material albedo" "diagnostics do not report product lighting layer"
}
