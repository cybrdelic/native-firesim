...
Require-Text $main "debugOverlaySystem=source markers, selected burner ports, volume bounds, fuel-bed bounds, origin axes, frame age, and texture ring freshness" "diagnostics debug overlay string is missing from source"
if (Test-Path $diag) {
    $diagText = Get-Content $diag -Raw
    Require-Text $diagText "debugOverlaySystem=source markers, selected burner ports, volume bounds, fuel-bed bounds, origin axes, frame age, and texture ring freshness" "diagnostics do not report debug overlay system"
}
